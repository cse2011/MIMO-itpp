/*
---------------------------------------------MIMO 2X1 ALAMOUTI STBC, CON MODULACIÓN QPSK------------------------------------------------
...
int N = 10000000;    //Número de bits a simular
int i, j, z;
int k, y;
int qpskLength;
...
Ec = 1.0;                       //La energía transmitida por símbolo en QPSK es 1
Eb = Ec / 2.0;                  //La energía transmitida por bit es 0.5
SNRdB = linspace(0,25,26);      //Vector de SNR en dB
EbN0dB = SNRdB - 10*log10(2.0);   //Se convierte el parámetro SNR a N0, para poder utilizar
...
//Se modula el vector bits y se guardan los símbolos en el vector qpskSymbols
qpsk.modulate_bits(in_bits, qpskSymbols);
qpskLength = qpskSymbols.length();        //Tamaño del vector de símbolos

//Dividimos el vector de símbolos en s0 y s1, un ejemplo a continuación:
//qamSymbols = [a b c d e f];
//s0 = [a c e]
//s1 = [b d f]
s0 = zeros_c(1,qpskLength/2);
s1 = zeros_c(1,qpskLength/2);
z = 0;
for (j=0; j < qpskLength; j = j+2){
    if (j == 0){
        s0(j) = qpskSymbols(j);
        s1(j) = qpskSymbols(j+1);
    }else {
        s0(z) = qpskSymbols(j);
        s1(z) = qpskSymbols(j+1);
    }
    z++;
}
...
h0 = 1/sqrt(2)*(randn(1,qpskLength/2)) + mycomplex * (randn(1,qpskLength/2));
h1 = 1/sqrt(2)*(randn(1,qpskLength/2)) + mycomplex * (randn(1,qpskLength/2));
...
for (i=0; i < SNRdB.length(); i++) {
    ...
    //Se demodula el vector qpskSymbols y se guardan esos bits en dec_bits
    qpsk.demodulate_bits(reciv, dec_bits);
...
cout<<"BER ="<< BER <<endl;
cout<<"--------------------------------------------------------------------------------"<<endl;
cout <<"Resultados guardados en ./MIMO_2X1_alamouti_qpsk_result_file.it" << endl;
cout << endl;



//Se almacenan los resultados de la simulación en un archivo .it:
ff.open("MIMO_2X1_alamouti_qpsk_result_file.it");
ff << Name("SNR_QPSK") << SNRdB;
ff << Name("BER_QPSK") << BER;
ff.close();
}
